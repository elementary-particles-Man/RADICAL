#![no_std]
#![no_main]
#![feature(abi_x86_interrupt)]

extern crate alloc;

use core::arch::asm;
use uefi::prelude::*;
use uefi::allocator::exit_boot_services;
use uefi::proto::console::gop::GraphicsOutput;

#[macro_use]
mod drivers;
mod arch;
mod mm;
mod compression;
mod task;
mod installer;
mod kernel_security;

use task::{Task, executor::Executor};
use crate::drivers::gpu::{GpuCommandRing, GopInfo, SCREEN_LOGGER};
use crate::drivers::virtio_blk::VirtioBlk;
use core::{future::Future, pin::Pin, task::{Context, Poll}};
use core::sync::atomic::Ordering;
use crate::drivers::pci::{PciAddress, PciBar};
use crate::arch::x86_64::{interrupts, cpu, gdt, apic, paging, syscall, user};
use crate::mm::memory;
use crate::compression::zram;

struct SleepFuture {
    target_tick: u64,
}

impl SleepFuture {
    fn new(ticks: u64) -> Self {
        let current = interrupts::current_tick();
        SleepFuture { target_tick: current + ticks }
    }
}

impl Future for SleepFuture {
    type Output = ();

    fn poll(self: Pin<&mut Self>, cx: &mut Context) -> Poll<()> {
        let current = interrupts::current_tick();
        if current >= self.target_tick {
            Poll::Ready(())
        } else {
            interrupts::register_timer_waker(self.target_tick, cx.waker());
            Poll::Pending
        }
    }
}

#[entry]
fn main(_image_handle: Handle, mut system_table: SystemTable<Boot>) -> Status {
    unsafe { drivers::serial::COM1.lock().init(); }

    // UEFI Graphics Output Protocol (GOP) Baseline
    let gop_info = system_table.boot_services()
        .find_first_capable_handle::<GraphicsOutput>()
        .ok()
        .and_then(|handle| {
            system_table.boot_services()
                .open_protocol_exclusive::<GraphicsOutput>(handle)
                .ok()
        })
        .map(|mut gop| {
            let mut fb = gop.frame_buffer();
            GopInfo {
                fb_base: fb.as_mut_ptr() as u64,
                fb_size: fb.size() as u64,
                width: gop.current_mode_info().resolution().0 as u32,
                height: gop.current_mode_info().resolution().1 as u32,
                stride: gop.current_mode_info().stride() as u32,
            }
        });

    serial_println!("--- RADICAL-KERNEL REBIRTH (FINAL TUNE) ---");

    kernel_security::assert_secret_fd_boundary_sealed();

    serial_println!("RADICAL-KERNEL: Requesting UEFI ExitBootServices handoff...");
    let (runtime_table, mut memory_map) = system_table.exit_boot_services();
    exit_boot_services();
    x86_64::instructions::interrupts::disable();
    serial_println!("RADICAL-KERNEL: ExitBootServices complete. Firmware boot services are offline.");

    memory_map.sort();
    memory::init_memory(&memory_map);
    memory::inspect_memory_map();
    unsafe { paging::init_paging(gop_info); }

    if let Some(info) = gop_info {
        SCREEN_LOGGER.lock().init(info);
        serial_println!("RADICAL-COMMANDER [GOP]: Screen Logger Online. Resolution: {}x{}", info.width, info.height);
    }
    
    let features = unsafe { cpu::init_simd() };
    cpu::log_features(&features);
    
    if !features.avx_enabled {
        serial_println!("RADICAL-KERNEL: [WARNING] AVX runtime unavailable. SIMD optimization degraded.");
    }

    serial_println!("RADICAL-KERNEL: Asserting absolute control over CPU (GDT/IDT)...");
    gdt::init();
    interrupts::init_idt();
    unsafe { syscall::init(); }
    let apic_topology = apic::init(&runtime_table);
    if let Some(topo) = apic_topology {
        unsafe {
            apic::disable_8259_pic();
            if !topo.io_apics.is_empty() {
                apic::io_apic_set_redirection(topo.io_apics[0].address, 1, 33, 0);
                serial_println!("RADICAL-APIC: Keyboard IRQ 1 routed via I/O APIC.");
            }
        }
    }
    interrupts::set_interrupt_timer_ready(apic::timer_routing_ready());
    zram::init();

    let mut executor = Executor::new();
    
    executor.spawn(Task::new(async_pcie_probe_and_init()));
    executor.spawn(Task::new(async_runtime_diagnostics(features)));

    serial_println!(
        "RADICAL-KERNEL: Spawning {} cooperative worker modules...",
        features.recommended_workers
    );
    for thread_id in 0..features.recommended_workers {
        executor.spawn(Task::new(async_worker_module(thread_id)));
    }

    if interrupts::interrupt_timer_ready() {
        serial_println!("RADICAL-KERNEL: APIC timer routing online. Releasing Interrupt Seals...");
        x86_64::instructions::interrupts::enable();
    } else {
        serial_println!("RADICAL-KERNEL: APIC timer routing pending. External IRQs stay masked; cooperative scheduler fallback active.");
    }
    serial_println!("RADICAL-KERNEL: OS Tick Active. Entering Async Executor loop.");

    executor.run();
}

async fn async_worker_module(thread_id: u32) {
    serial_println!("RADICAL-ASYNC [WORKER-{}]: Online. Awaiting Vulkan/SIMD tasks.", thread_id);
    let base_sleep = 50 + (thread_id as u64 * 15);
    loop {
        SleepFuture::new(base_sleep).await;
        let current_tick = interrupts::TICKS.load(Ordering::Relaxed);
        if current_tick.is_multiple_of(1000) {
            serial_println!("RADICAL-ASYNC [WORKER-{}]: Heartbeat. OS Tick: {}", thread_id, current_tick);
        }
    }
}

async fn async_pcie_probe_and_init() {
    serial_println!("RADICAL-ASYNC [INIT]: Asynchronous PCIe probing for GPU/Storage...");
    let mut gpu_mmio_base: Option<u64> = None;
    let mut storage_device: Option<VirtioBlk> = None;

    for bus in 0..=255 {
        for slot in 0..=31 {
            let address_base = PciAddress { bus, slot, func: 0 };
            let dev_base = unsafe { drivers::pci::probe_device(address_base) };
            if dev_base.is_none() { continue; }
            
            for func in 0..=7 {
                let address = PciAddress { bus, slot, func };
                let Some(dev) = (unsafe { drivers::pci::probe_device(address) }) else { continue; };
                
                if dev.class == 0x03 { 
                    if let Some(bar0) = unsafe { read_pci_bar0(address) } {
                        gpu_mmio_base = Some(bar0);
                        if let Some(vgpu) = unsafe { drivers::virtio_gpu::VirtioGpu::from_pci(address) } {
                            unsafe { vgpu.init(); }
                        }
                    }
                }

                if dev.vendor_id == 0x1AF4 { 
                    if dev.device_id == 0x1001 { 
                        if let Some(device) = unsafe { VirtioBlk::from_pci(address) } {
                            storage_device = Some(device);
                        }
                    }
                }
            }
        }
        if bus % 32 == 0 { SleepFuture::new(1).await; }
    }

    if let Some(base) = gpu_mmio_base {
        serial_println!("RADICAL-ASYNC [INIT]: GPU Active at 0x{:x}. Submitting Vulkan-compatible pipeline.", base);
        let _ring = unsafe { GpuCommandRing::new(base, 0x5000000) };
    } else {
        serial_println!("RADICAL-ASYNC [INIT]: No GPU found. Directing Vulkan workload to CPU SIMD fallback.");
        async_cpu_simd_fallback_task().await;
    }

    if let Some(disk) = storage_device {
        async_install_task(disk).await;
    }
}

async fn async_runtime_diagnostics(features: cpu::CpuFeatures) {
    SleepFuture::new(5).await;
    serial_println!(
        "RADICAL-ASYNC [CPU]: workers={} simd={} avx={} avx512={} xcr0={:#x}",
        features.recommended_workers,
        features.simd_enabled,
        features.avx_enabled,
        features.avx512_enabled,
        features.xcr0
    );
}

async fn async_gpu_compute_task(mut ring: GpuCommandRing) {
    serial_println!("RADICAL-ASYNC [GPU]: Vulkan compute sequence isolated.");
    SleepFuture::new(10).await;
    
    let shader_id = 0x70FF;
    let data_phys = 0x6000000;
    let result_phys = 0x6001000;

    serial_println!("RADICAL-ASYNC [GPU]: Submitting batch to Command Ring...");
    if ring.submit_compute_batch(shader_id, data_phys, result_phys) {
        ring.wait_for_idle();
        serial_println!("RADICAL-ASYNC [GPU]: Batch completed via Vulkan path.");
    } else {
        serial_println!("RADICAL-ASYNC [GPU]: Submission rejected. Falling back to CPU SIMD.");
        async_cpu_simd_fallback_task().await;
    }
}

async fn async_cpu_simd_fallback_task() {
    serial_println!("RADICAL-ASYNC [CPU-FALLBACK]: Executing Vulkan compute workload via SIMD.");
    SleepFuture::new(5).await;
    serial_println!("RADICAL-ASYNC [CPU-FALLBACK]: SIMD compute task finalized.");
}

async fn async_install_task(disk: VirtioBlk) {
    serial_println!("RADICAL-ASYNC [INSTALL-TASK]: Beginning automated deployment...");
    installer::run_install_pipeline(&disk);
    serial_println!("RADICAL-ASYNC [INSTALL-TASK]: Deployment finalized. System ready.");
}

#[panic_handler]
fn panic(info: &core::panic::PanicInfo) -> ! {
    serial_println!("\n[!!!] RADICAL-KERNEL PANIC [!!!]");
    serial_println!("Nature: {}", info);
    serial_println!("System halted. The core remains pure.");
    loop { unsafe { asm!("hlt"); } }
}

unsafe fn read_pci_bar0(address: PciAddress) -> Option<u64> {
    match drivers::pci::read_bar(address, 0)? {
        PciBar::Memory32 { base, .. } | PciBar::Memory64 { base, .. } => Some(base),
        PciBar::Io { .. } => None,
    }
}

async fn async_user_handoff() {
    serial_println!("RADICAL-ASYNC [USER]: Initiating handoff to unprivileged space.");
    SleepFuture::new(10).await; 
    user::spawn_user_hello();
}
